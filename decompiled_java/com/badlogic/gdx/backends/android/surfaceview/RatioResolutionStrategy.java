...
public class RatioResolutionStrategy implements ResolutionStrategy {
    ...
    public MeasuredDimension calcMeasures(int widthMeasureSpec, int heightMeasureSpec) {
        int v1;
        int v5;
        int v4 = View$MeasureSpec.getSize(widthMeasureSpec);
        int v3 = View$MeasureSpec.getSize(heightMeasureSpec);
        float v0 = this.ratio;
        if((((float)v4)) / (((float)v3)) < v0) {
            v5 = v4;
            v1 = Math.round((((float)v5)) / v0);
        }
        else {
            v1 = v3;
            v5 = Math.round((((float)v1)) * v0);
        }

        return new MeasuredDimension(v5, v1);
    }
}
